...
if [ ! -z "$which_android" ]; then
  ...
  sudo sh -c "echo \"export PATH=$PATH:/usr/local/bin/android-sdk-linux/tools:/usr/local/bin/android-sdk-linux/platform-tools\" > /etc/profile.d/android-sdk-path.sh"
